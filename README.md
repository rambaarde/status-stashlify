...
5. A separate Pages deploy workflow publishes the static site from `main`.
6. The status UI reads `public/status/current.json` first, then falls back to `public/status.json`, then to the live uptime API, then to a neutral no-data state.
